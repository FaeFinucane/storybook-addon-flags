...
import { IconButton, ListItem, WithTooltipPure, TooltipLinkList, Spaced, Span } from "@storybook/components";
...
export const Tool = memo(function FlagSelector() {
  ...
  const list = useCallback(({ onHide }: { onHide(): void }) => (
    <List>
      {Object.entries(flagOptions).map(([name, flag]) => {
        const FlagListItem = flag.type === 'boolean' ? BoolFlagListItem : EnumFlagListItem;
        return (
          <FlagListItem
            key={name}
            name={name}
            ...
            onUpdate={(value) => {
              updateGlobals({ [PARAM_KEY]: { ...flagValues, [name]: value } });
              onHide();
            }}
          />
        );
...
type BoolFlagItemProps = { name: string, flag: BoolFlag, value?: string, onUpdate: (value: string) => void }
const BoolFlagListItem = memo(({ name, flag, value, onUpdate }: BoolFlagItemProps) => {
  const actualValue = value !== undefined ? (value === "true") : flag.defaultValue;
  return (
    <ListItem
      id={name}
      title={name}
...
type EnumFlagItemProps = { name: string, flag: EnumFlag, value?: string, onUpdate: (value: string) => void }
const EnumFlagListItem = memo(({ name, flag, value, onUpdate }: EnumFlagItemProps) => {
  ...
  return (
    ...
    >
      <ListItem
        id={name}
        title={name}
        right={<><ValueLabel>{actualValue}</ValueLabel><ChevronRightIcon fill="black" /></>}
      />
    </WithTooltipPure>
  )
})
